...
const getAllContinent = async (req, res) => {
    try {
      const continent = await continentModel.find() .populate('Countries'); ; // Retrieve all continent
  ...
  const getContinentById = async (req, res) => {
    try {
      ...
      const Continent = await continentModel.findById(ContinentId).populate("Countries"); // Find Continent by ID
  ...
  const deleteContinent = async (req, res) => {
    try {
      const ContinentId = req.params.id; // Get Continent ID from URL params
      const deletedContinent = await continentModel.findByIdAndDelete(ContinentId); // Delete the Continent
